...
*   **Epic 13: Android Companion Widget.** A native Android home-screen widget (Kotlin) so coding-habit tracking doesn't require opening a laptop — streak status/urgency, one AI-recommended problem for today, XP/level as a secondary glance, tap-to-open the problem. Requires native `AppWidgetProvider`/Jetpack Glance APIs; not achievable from the web app. Build scope (full companion app vs. widget-only) is an open decision, to be made when this epic is actually picked up. Can be scaffolded early against mocks, but needs Epics 4, 6, and 9 functional for live data.
